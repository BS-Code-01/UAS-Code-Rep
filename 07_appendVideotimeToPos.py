...
root = "D:/UAV-Befliegungen/210904_zuHauseNebenan/02_logs/Versuch_211105/"
...
# Abfrage der Systemzeit seit Boot (TimeUS)
# beim Starten und Stoppen der Videoaufnahme (Rückgabetyp: Tupel).
BARO_dict = log_file.extract_BARO_from_log()
BARO_list = list(BARO_dict.keys())
    ...
    with open(csv_file_read, "r") as csv_file_read_opened:
        ...
        for row in csv_file_read_opened.readlines()[1:]:
            ...
            try:
                ...
                ix = closest_value(BARO_list, timeUS)
                BARO = round(float(BARO_dict[str(ix)][0]) + 263.64, 4)
                print("success: matching BARO-record found")
                print("BARO: " + str(BARO))
